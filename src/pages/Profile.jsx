...
import { deleteProfileUser, getPostsByUser, getProfileUser, updateProfileUser } from "../request"
...
const fetchProfile=async ()=>{
  try{
     const res= await getProfileUser(user?.id)
...
const handleUserUpdate=async ()=>{
  setUpdated(false)
  try{
    const res = await updateProfileUser(user?.id,{username,email,password})
...
const handleUserDelete=async()=>{
  try{
    const result = await deleteProfileUser(user?.id)
    setUser(null)
    navigate("/")
    // console.log(res.data)
  }
